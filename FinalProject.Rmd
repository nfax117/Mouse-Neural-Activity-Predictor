...
```{r}
# Create general plot so that we can use clustering to find the specific sessions that we can combine into one dataframe
# Do this for different variables (eg. proportion of active neurons, etc)
# This new dataframe will hold the values that provide information for a better predictie model

# For part 2, combine specific sessions that give us information
# For example, if session 2 and 19 shows both similar sharp slopes, combine these two dataframes
...
```{r}
# TODO: 
# for part 2, figure out which variables to include in the model for part 3. 
# use DT::datatable() to show all dataframe variables, do this for original dataframe and dataframe that we chose for predictive model (this one will have less variables and rows)

# I will use variables: firing_rate, number_of_active_neurons, total_number_of_neurons, number_of_unique_brain_area, contrast_difference. (proportion of active neurons is too similiar to firing rate, and total_number_of_spks is too similar to number of active neurons. )
