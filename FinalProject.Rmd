...
abstract: |
  In this project, we were given 18 sessions of mice brain neural activity, each session recording brain activity related variables as well as the feedback produced from the mice after completing a visual discrimination task. With this information, data was combined into multiple dataframes and important variables were classified. Clustering and classification techniques were used in order to identify similar data across different trials. Logistic Regression was then applied to the complete dataframe that holds our predicting variabes in order to form our predictive model.
---
...
sessionDF.k3 <- subset(sessionDF, select = -c(session_number, trial_number, mouse_name, feedback_type)) %>% kmeans(3)
...
sessionDF.logreg <- glm(as.factor(feedback_type) ~ cluster_id + contrast_left + contrast_right + total_number_of_neurons + number_of_unique_brain_area + number_of_trials + contrast_difference + number_of_active_neurons + total_number_of_spks + proportion_of_active_neurons + firing_rate, data = sessionDF_train, family = "binomial")

summary(sessionDF.logreg)
...
paste("Accuracy is: ", logRegpredictedModel.cm$overall['Accuracy'])



# compare model with brain area and without
...
# May need more variables for prediction model to improve accuracy
# Use another model and use table to compare different model accuracies
...
SVMPredictedModel = svm(formula = as.factor(feedback_type) ~ cluster_id + contrast_left + contrast_right + total_number_of_neurons + number_of_unique_brain_area + number_of_trials + contrast_difference + number_of_active_neurons + total_number_of_spks + proportion_of_active_neurons + firing_rate,
...
SVMPredictedModel.cm

paste("Accuracy is: ", SVMPredictedModel.cm$overall['Accuracy'])
