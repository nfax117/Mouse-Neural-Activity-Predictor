...
for (i in 1:length(mouseNameVector)) {
  ...
  tempBrainAreaPerMouse <- completeDF[completeDF$mouse_name == mouseNameVector[i], ]
  ...
  ggplot(melt(tempBrainAreaPerMouse), aes(x = variable, y = value)) + geom_boxplot(outlier.color="red", outlier.shape=8, outlier.size=2, alpha = 0.2) + xlab("Brain Areas") + ylab("Firing Rate") + ggtitle(paste("Mouse", mouseNameVector[i], "Box Plots of Brain Area Firing Rates"))
